...
use spin_factors::{
  App, RuntimeFactors,
  anyhow::{self, Context},
  wasmtime::{Config, Engine, component::Linker},
};
...
impl<T: RuntimeFactors> TestEnvironment<T> {
  ...
  pub fn new(mut factors: T) -> Self {
    ...
    let engine =
      Engine::new(Config::new().async_support(true)).expect("wasmtime engine failed to initialize");
  ...
  //     .components()
  //     .last()
  //     .context("expected configured app to have at least one component, but it did not")?;
  ...
  pub async fn build_locked_app(&self) -> anyhow::Result<LockedApp> {
    ...
    return spin_loader::from_file(&path, FilesMountStrategy::Direct, None).await;
  }
}
